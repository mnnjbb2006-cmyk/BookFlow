...
class Ui_MainWindow(object):
    ...
    def setupActions(self):
        ...
        self.pushButtonEnable.setEnabled(False)
        self.pushButtonEnable.clicked.connect(self.enable_user)
        self.pushButtonDisable.setEnabled(False)
...
#####################           /functions/                   ###################################################################################################################################################
    ...
    def user_selection_changed(self):
        try:
            selected_row = self.tableWidgetUsers.currentRow()
            if selected_row != -1:
                self.selected_username = self.tableWidgetUsers.item(selected_row, 0).text()
                self.pushButtonDelete.setEnabled(True)
                self.pushButtonEnable.setEnabled(True)
                self.pushButtonDisable.setEnabled(True)
            else:
    ...
    def refresh_users(self):
        try:
            self.tableWidgetUsers.setRowCount(0)
            for rowPosition, user in enumerate(users.getusers()):
